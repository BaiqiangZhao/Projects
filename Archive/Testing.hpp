...
namespace ECE141 {
    ...
    struct Testing : public ArchiveObserver {
        ...
        bool doCreateTests(std::ostream& anOutput) {
...

          {
            ...
            Archive* theArchive2 = Archive::createArchive(folder + "/test");
            if (theArchive2) {
              if (!fs::exists(folder + "/test.arc")) { return false; }
            }
            else { return false; }

            Archive* theArchive3 = Archive::createArchive(folder + "/test2");
            if (theArchive3) {
        ...
        bool doOpenTests(std::ostream& anOutput) {
            ...
            theTracker.enable(true).reset();
            {
              Archive* theArchive = Archive::createArchive(folder + "/test");
              if (theArchive) {
                theArchive->add(folder + "/small.txt");
                Archive* newArchive = Archive::openArchive(folder + "/test");
                if (newArchive) {
        ...
        bool doAddTests(std::ostream& anOutput) {
...

          {
            ...
            std::uintmax_t preSize{0}, postSize{0};
            
            std::string theFullPath(folder+"/addtest.arc");
            Archive* theArchive = Archive::createArchive(theFullPath);
            if (theArchive) {
        ...
        bool doExtractTests(std::ostream& anOutput) {
          ...
          {
            bool theResult = false;
            Archive* theArchive = Archive::createArchive(folder + "/extracttest");
            if (theArchive) {
              theArchive->addObserver(*this);
